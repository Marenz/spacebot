...
function GenericProviderIcon({ size = 24, className }: IconProps) {
	return (
		<svg
			...
			focusable="false"
		>
			<rect x="3" y="3" width="18" height="18" rx="3" stroke="currentColor" strokeWidth="1.5" strokeOpacity="0.5" />
			<circle cx="8" cy="8" r="1.5" fill="currentColor" fillOpacity="0.5" />
			<circle cx="16" cy="8" r="1.5" fill="currentColor" fillOpacity="0.5" />
			<circle cx="8" cy="16" r="1.5" fill="currentColor" fillOpacity="0.5" />
...
export function ProviderIcon({ provider, className = "text-ink-faint", size = 24 }: ProviderIconProps) {
	...
	if (!IconComponent) {
		return <GenericProviderIcon {...iconProps} />;
	}
